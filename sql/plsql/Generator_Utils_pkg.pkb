...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "GENERATOR_UTILS" AS
    ...
    END;
        ...
        PROCEDURE generate_updates(pTbl in varchar2) is
    -- --------------------------------------------------------------------------
        v_updatetxt VARCHAR2(4000);
        v_datatxt   VARCHAR2(4000);  
        v_v_val     VARCHAR2(4000);
        v_n_val     NUMBER;
        v_d_val     DATE;
    ...
    BEGIN
        ...
        END LOOP;
        -- ----------------------------------------------------------------------
        -- This part generates the update columns
            v_updatetxt := NULL;
        for j in 1..col_cnt
        LOOP
            v_updatetxt := ltrim(v_updatetxt||','||lower(rec_tab(j).col_name),',');
        END LOOP;
            v_updatetxt := 'insert into '||pTbl||' ('||v_updatetxt||') values (';
        -- ----------------------------------------------------------------------    
        -- This part outputs the DATA
        LOOP
            v_ret := DBMS_SQL.FETCH_ROWS(c);
